"""Drop-in 6-D variant of run_pullback_experiment.py.

Loads data/manifold_h_6d_full.npz instead of the default 8-D
production manifold. Writes results to results/pullback_6d/
and data/pullback_6d/ so 8-D and 4-D artifacts are not overwritten.
...
OUT_DATA_DIR = Path("data/pullback_6d")
OUT_RESULTS_DIR = Path("results/pullback_6d")
...
async def main() -> None:
    ...
    config = load_config()
    if not MANIFOLD_6D_PATH.exists():
        raise SystemExit(
            f"Missing {MANIFOLD_6D_PATH}. Run scripts/setup_6d_pipeline.py first."
        )
    manifold = FittedManifold.load(MANIFOLD_6D_PATH)
    ...
    for start, end in pairs:
        print()
        print(f"=== 6-D pullback experiment: {start} → {end}  (K={args.k}, N={args.n}) ===")
