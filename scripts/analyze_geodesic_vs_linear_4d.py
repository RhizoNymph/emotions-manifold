"""4-D variant of analyze_geodesic_vs_linear.py.

Reads results/pullback_4d/{s}_{e}.json (produced by
scripts/alift_4d_chain.sh) and computes the geodesic-vs-linear
and pullback-vs-linear comparisons, mirroring the 8-D analysis at
analyze_geodesic_vs_linear.py.

Output: results/riemannian_analysis_4d/_summary.json + figures.
"""
...
RESULTS_DIR = Path("results/pullback_4d")
...
def main():
    ...
    print(f"\nLoaded {len(rows)}/{len(all_predictions)} pairs at 4-D")

    if len(rows) == 0:
        print("\nNo pairs loaded. Did you run scripts/alift_4d_chain.sh?")
        return
    ...
    print("\n=== 4-D RESULTS (compare to 8-D in results/riemannian.md) ===")
